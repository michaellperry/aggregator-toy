...
/**
 * A step that computes the minimum value of a property over items in a nested array.
 * 
 * - Returns undefined for empty arrays
 * - Ignores null/undefined values in comparison
 * - Handles removal by tracking all values and recalculating min
 */
export class MinAggregateStep<
    TInput,
...
> implements Step {
    ...
    private handleItemAdded(runtimePath: string[], itemKey: string, item: ImmutableProps): void {
        ...
        // Compute new min
        const values = this.valueStore.get(parentHash) || [];
        const newMin = values.length > 0 ? Math.min(...values) : undefined;
        ...
        if (parentPath.length > 0) {
            ...
            this.modifiedHandlers.forEach(({ handler }) => {
                handler(pathToParent, parentKey, this.propertyName, newMin);
            });
        } else {
            // Parent is at root level
            this.modifiedHandlers.forEach(({ handler }) => {
                handler([], '', this.propertyName, newMin);
            });
        }
    }
